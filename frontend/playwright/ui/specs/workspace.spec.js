...
test("PodConverge hides layout creation actions from shape context menu", async ({
  page,
}) => {
  const workspacePage = new WasmWorkspacePage(page);
  await workspacePage.setupEmptyFile(page);
  ...
  await expect(
    workspacePage.contextMenuForShape.getByText("Add flex layout", {
      exact: true,
    }),
  ).toHaveCount(0);
  await expect(
    workspacePage.contextMenuForShape.getByText("Add grid layout", {
      exact: true,
    }),
  ).toHaveCount(0);
});
